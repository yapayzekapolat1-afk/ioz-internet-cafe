        ...
        bridge.showRewarded({ placementId: AD_PLACEMENT_ID })
          ...
          .catch(function (err) {
            ...
            var reasons = {
              ...
              init_timeout: "Reklam sistemi hiç yanıt vermedi (25sn) — internet bağlantını kontrol et.",
              show_failed: "Reklam gösterimi başarısız oldu.",
              skipped: "Reklam tamamlanmadan kapatıldı."
            };
            var msg = reasons[raw];
            if (!msg && raw.indexOf("init_failed:") === 0) {
              msg = "Unity Ads başlatma hatası — " + raw.replace("init_failed: ", "");
            }
            if (!msg) msg = "Reklam hatası: " + raw;
            showToast(msg, 6000);
            onDone(false);
          });
        return;
